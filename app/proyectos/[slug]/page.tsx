...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header /><Cursor /><main className="case-study">
    ...
    {project.gallery.length > 0 && <section className="case-gallery" aria-label={`Identidad visual de ${project.name}`}>{project.gallery.map((image, index) => <figure key={image}><div><Image src={image} alt={`Identidad visual de ${project.name}`} fill sizes="(max-width: 760px) 100vw, 80vw" /></div><figcaption className="utility">{String(index + 1).padStart(2, "0")} / Identidad de marca</figcaption></figure>)}</section>}
    ...
    {project.slug === "post-produccion" && <section className="media-placeholder"><p className="utility">Portafolio audiovisual</p><h2>TV · REELS · MOTION<br />COMERCIAL · CONTENIDO</h2><p>Una selección dedicada de edición, narrativa, composición y piezas multiplataforma.</p><Link href="/portafolio/postproduccion">Ver portafolio de postproducción ↗</Link></section>}
